...
export function Settings() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setAutoDiscoveryEnabled(runtime.health.auto_discovery_enabled);
        setRuntimeNote(runtime.input_status?.note ?? "运行状态正常");
        setLogDir(logPreview.log_path);
        setLogLines(logPreview.lines);
        setStatusText("设置已同步到当前设备。");
      } catch (nextError) {
  ...
  async function handleClipboardToggle() {
    ...
    try {
      ...
      setStatusText(next ? "已开启剪切板共享。" : "已关闭剪切板共享。");
    } catch (nextError) {
      setError(nextError instanceof Error ? nextError.message : String(nextError));
    }
  }
  ...
  async function handleExportDiagnostics() {
    try {
      ...
      setStatusText(`诊断快照已导出到 ${result.path}`);
    } catch (nextError) {
      setError(nextError instanceof Error ? nextError.message : String(nextError));
    }
  }
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="custom-scrollbar flex-1 space-y-4 overflow-y-auto pb-6 pr-2">
        <div className="rounded-lg border border-slate-700/60 bg-slate-800/80 p-4 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <div className="mb-1.5 flex justify-between">
                <label className="flex items-center gap-1.5 text-[11px] font-medium text-slate-300">
                  <MousePointer2 className="text-slate-400" size={12} />
                  鼠标指针速度
                </label>
            ...
            <div>
              <div className="mb-1.5 flex justify-between">
                <label className="flex items-center gap-1.5 text-[11px] font-medium text-slate-300">
                  <Mouse className="text-slate-400" size={12} />
                  滚轮速度
                </label>
            ...
            <div>
              <div className="mb-1.5 flex justify-between">
                <label className="flex items-center gap-1.5 text-[11px] font-medium text-slate-300">
                  <MoveVertical className="text-slate-400" size={12} />
                  滚轮平滑度
                </label>
        ...
        <div className="rounded-lg border border-slate-700/60 bg-slate-800/80 p-4 shadow-sm">
          ...
          <div className="space-y-3.5">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-[11px] font-medium text-slate-200">剪切板共享</h4>
                <p className="mt-0.5 text-[10px] text-slate-500">主副设备之间同步文本剪切板内容。</p>
              </div>
              <button
            ...
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-[11px] font-medium text-slate-200">自动发现</h4>
                <p className="mt-0.5 text-[10px] text-slate-500">当前为服务侧配置状态，仅展示，不在此页面直接切换。</p>
              </div>
              <div className={`rounded-full px-2.5 py-1 text-[10px] font-medium ${autoDiscoveryEnabled ? "bg-emerald-400/10 text-emerald-400" : "bg-slate-700 text-slate-300"}`}>
                {autoDiscoveryEnabled ? "已开启" : "已关闭"}
              </div>
            </div>

            <div className="rounded-md border border-slate-700/60 bg-slate-900/60 p-3 text-[10px] text-slate-400">
              {runtimeNote}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-slate-700/60 bg-slate-800/80 p-4 shadow-sm">
          ...
          <div className="space-y-3.5">
            <div>
              <h4 className="mb-1 text-[11px] font-medium text-slate-200">日志路径</h4>
            ...
            <div className="flex gap-2">
              <button
                className="rounded-md bg-blue-600 px-3 py-2 text-[10px] font-medium text-white transition-colors hover:bg-blue-500"
              ...
              {diagnosticPath ? (
                <div className="flex items-center rounded-md border border-slate-700 bg-slate-900/70 px-3 py-2 text-[10px] text-slate-400">
                  {diagnosticPath}
                </div>
              ) : null}
            </div>
